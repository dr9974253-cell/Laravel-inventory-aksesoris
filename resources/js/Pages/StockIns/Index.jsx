...
export default function Index({ stockIns, filters }) {
    const { props } = usePage();
    ...
    const handleSearch = (e) => {
        ...
        window.location.href = route('stock-ins.index') + '?' + params.toString();
    };

    const confirmDelete = (id, name) => {
        if (window.confirm(`Apakah Anda yakin ingin menghapus data barang masuk "${name}"?`)) {
            document.getElementById('delete-form-' + id).submit();
        }
    };

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Barang Masuk</h2>}>
            ...
            <div className="p-6">
                ...
                <div className="rounded-xl bg-white shadow-sm">
                    ...
                    <div className="flex flex-col gap-4 border-b border-gray-200 p-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        {isAdmin && (
                            <Link href={route('stock-ins.create')} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
                                <Plus className="h-4 w-4" /> Tambah Barang Masuk
                            </Link>
                        )}
                    </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {stockIns.data.length === 0 ? (
                                    <tr><td colSpan={isAdmin ? 8 : 6} className="px-4 py-8 text-center text-sm text-gray-500">Tidak ada data barang masuk.</td></tr>
                                ) : (
                                    stockIns.data.map((item, index) => (
                                        <tr key={item.id} className="hover:bg-gray-50">
                                            <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-900">{stockIns.from + index}</td>
                                            ...
                                            <td className="whitespace-nowrap px-4 py-3 text-center"><span className="inline-flex rounded-full bg-green-100 px-2 py-1 text-xs font-medium text-green-700">{item.jumlah}</span></td>
                                            ...
                                            {isAdmin && (
                                                <td className="whitespace-nowrap px-4 py-3 text-center">
                                                    ...
                                                    <form id={`delete-form-${item.id}`} method="POST" action={route('stock-ins.destroy', item.id)} className="hidden">
                    ...
                    {stockIns.last_page > 1 && (
                        <div className="flex items-center justify-between border-t border-gray-200 px-4 py-3">
                            <div className="text-sm text-gray-500">Menampilkan {stockIns.from} sampai {stockIns.to} dari {stockIns.total} data</div>
