...
export default function Create({ products }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        router.post(route('stock-ins.store'), values);
    };

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Tambah Barang Masuk</h2>}>
            <Head title="Tambah Barang Masuk" />
            <div className="p-6">
                <div className="mx-auto max-w-2xl rounded-xl bg-white p-6 shadow-sm">
                    {/* Header */}
                    <div className="mb-6 flex items-center gap-4 border-b border-gray-200 pb-4">
                        <Link href={route('stock-ins.index')} className="flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900">
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="grid gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="number" name="jumlah" min="1" value={values.jumlah} onChange={handleChange} className={`w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-blue-500 ${errors?.jumlah ? 'border-red-500' : 'border-gray-300'}`} />
                            ...
                            <div>
                                ...
                                <input type="text" name="no_referensi" value={values.no_referensi} onChange={handleChange} className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500" placeholder="Contoh: Nota 001" />
                            </div>
                        ...
                        <div className="mt-6 flex items-center justify-end gap-3 border-t border-gray-200 pt-6">
                            <Link href={route('stock-ins.index')} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Batal</Link>
